...
func CreateBST(intSlice []int) ([]string, []string, []string, result.TreeResults) {
	var bst ItemBinarySearchTree

	for i := 0; i < len(intSlice); i++ {
		bst.Insert(intSlice[i], intSlice[i])
	}
	...
	bst.InOrderTraverse(func(i Item) {
		inOrder = append(inOrder, fmt.Sprintf("%d", i))
	})

	var preOrder []string
	bst.PreOrderTraverse(func(i Item) {
		preOrder = append(preOrder, fmt.Sprintf("%d", i))
	})

	var postOrder []string
	bst.PostOrderTraverse(func(i Item) {
		postOrder = append(postOrder, fmt.Sprintf("%d", i))
	})

	return inOrder, preOrder, postOrder, result.TreeResults{InsertAverageCase: "O[logN]", InsertWorstCase: "O[N]", SearchAverageCase: "O[logN]", SearchWorstCase: "O[N]", DeleteAverageCase: "O[logN]", DeleteWorstCase: "O[N]", Space: "O[N]"}
}
